    ...
    <div className="w-80 border-l border-white/10 bg-[#0A0A0F] flex flex-col h-full shadow-2xl relative z-40 transform transition-transform duration-300 ease-in-out">
      ...
      <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white text-sm">Workspace Chat</h2>
        </div>
              ...
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                {showHeader && (
                  <div className={`flex items-center gap-1.5 mb-1 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                    <span className="text-[10px] font-semibold text-slate-400 uppercase">{isMe ? 'You' : (msg.sender.username ? `@${msg.sender.username}` : msg.sender.name || 'User')}</span>
      ...
      <div className="p-3 border-t border-white/10 shrink-0 bg-[#0A0A0F] relative">
        {/* Mentions Dropdown */}
        {showMentionMenu && filteredMembers.length > 0 && (
          <div className="absolute bottom-full left-3 right-3 mb-2 bg-[#1E1E2D] border border-white/10 rounded-lg shadow-xl overflow-hidden z-50 max-h-48 overflow-y-auto">
            {filteredMembers.map(m => (
              ...
              >
                ...
                <div className="flex flex-col overflow-hidden">
                  <span className="text-sm text-white truncate">{m.user.username}</span>
                  <span className="text-[10px] text-slate-400 truncate">{m.user.name}</span>
                </div>
              </button>
            ))}
          </div>
            ...
            {filteredDocs.map(d => (
              ...
              >
                ...
                <div className="flex flex-col overflow-hidden">
                  ...
                  <span className="text-[10px] text-blue-400">Document</span>
                </div>
              </button>
            ))}
            {filteredFiles.map(f => (
              ...
              >
                ...
                <div className="flex flex-col overflow-hidden">
                  ...
                  <span className="text-[10px] text-green-400">File ({Math.round(f.size / 1024)} KB)</span>
                </div>
              </button>
            ))}
          </div>
